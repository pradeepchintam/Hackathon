...
describe('Starting tests for tutorial-network-app', function() {
  ...
  it('website title should be tutorial-network-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tutorial-network-app');
    })
  });
  ...
  it('navbar-brand should be tutorial-network-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorial-network-app');
    });
  });
