...
describe('Starting tests for GBT_Hotels_Network_UI', function() {
  ...
  it('website title should be GBT_Hotels_Network_UI', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('GBT_Hotels_Network_UI');
    })
  });
  ...
  it('navbar-brand should be GBT_Hotels_Network_UI',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('GBT_Hotels_Network_UI');
    });
  });
